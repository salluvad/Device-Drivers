...
/* the driver wull apper as "/dev/cedevdrv" */
...
/* these are used to send as args when we load using insmod */
static char *cedrvarg = "cedriver";  // since global variables are kernel wide , use static to restrict visibility
module_param(cedrvarg, charp, S_IRUGO);   //charp pointer type
MODULE_PARM_DESC(cedrvarg,"To use with printk");

static int majorNumber;    // device driver identifer , let it be set auto , or you can set it using sudo mknod /de                             v/test c 66 1 <char device , major and minor>
